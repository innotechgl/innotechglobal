...
final class gallery_connector_class extends util_class
{

    public $table = 'gallery_connector';
    public $id = 0;
    public $gal_id = 0;
    public $rel_id = 0;
    public $rel_page = '';
    ...
    public function add($rel_page, $rel_id, $gal_ids = array())
    {
        ...
        if (count($gal_ids) > 0) {
            foreach ($gal_ids as $gal_id) {
                // Add them
                $this->add_new($rel_id, $gal_id, $rel_page);
            }
        }
    }
     ...
     *
     */
    public function add_new($rel_id, $gal_id, $rel_page)
    {
        global $engine;
        // Prepare VALS
        $this->rel_id = (int)$rel_id;
        ...
        $this->rel_page = (string)$rel_page;
        // CREATE QUERY
        $query = 'INSERT INTO ' . $this->table . '(rel_id, gal_id, rel_page) VALUES (' . $this->rel_id . ', ' . $this->gal_id . ', "' . $this->rel_page . '");';
        // INSERT INTO BASE
    ...
    public function getGalIdsForConnection($rel_page, $rel_id)
    {
        global $engine;
        $this->rel_id = $engine->security->get_val($rel_id);
        $this->rel_page = $engine->security->get_val($rel_page);
        $query = "SELECT gal_id FROM " . $this->table . " WHERE rel_id=" . $this->rel_id . " AND rel_page LIKE '" . $this->rel_page . "';";
        $engine->dbase->query($query);
        $rows = $engine->dbase->rows;
        $ids = array();
        foreach ($rows as $val) {
