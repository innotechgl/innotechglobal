<?php
$engine->load_page('gallery');
$connected_gals = $engine->util_gallery_connector->get_array(false, '*', 'WHERE rel_id=' . (int)$rel_id . ' AND rel_page LIKE "' . $page . '"');
$gal_ids = array();
foreach ($connected_gals as $key_g => $val_g) {
    $gal_ids[] = $val_g['gal_id'];
}
if (count($gal_ids) > 0) {
    $photos = $engine->gallery->get_array(false, '*', 'WHERE categorie_id IN (' . implode(",", $gal_ids) . ')');
    if (count($photos) > 0) {
        ?>
        <style type="text/css">
