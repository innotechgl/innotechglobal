<?php
class modelArticlePhotosCategory extends article_photo_categories_class{
    ...
    public function _add($title, $parentID){
        ...
        mkdir(_APP_DIR_."media/images/article_photos/".$id);
    ...
    protected function _removePicturesFromCategories(Array $categories){
        $this->engine->load_page('article_photos');

        $idsToRemove = array();
        $filesToRemove = array();

        $photos = $this->engine->article_photos->get_array(false,"id,name,categorie_id","WHERE categorie_id IN (".
            implode(",",$categories).")");
        ...
        if (count ($idsToRemove) > 0){
            $query = "DELETE FROM ".$this->engine->article_photos->table." WHERE id IN (".implode(",",$idsToRemove).")";
            $this->engine->dbase->insertQuery($query);
        }
